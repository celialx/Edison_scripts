function varargout=sigstar(groups,stats,nosort, higherBar)
    ...
    y=ylim;
    if higherBar ==1
            yd=myRange(y)*0.15; %separate sig bars vertically by 5% 
    else
...
%     H(1)=plot(x(:),y,'-k','LineWidth',1.5,'Tag','sigstar_bar');
    H(1)=plot(x(:),y,'-k','LineWidth',2.5,'Tag','sigstar_bar', 'Color', [0.2 0.2 0.2]);
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars', 'FontSize', 30, 'FontName', 'Helvetica', 'Color', [0.2 0.2 0.2]);
    else
        H(2)=text(mean(x(:)),starY,stars,...
        'HorizontalAlignment','Center',...
        'BackGroundColor','none',...
        'Tag','sigstar_stars', 'FontSize', 15, 'FontName', 'Helvetica', 'Color', [0.2 0.2 0.2], 'FontWeight', 'bold');
    end
