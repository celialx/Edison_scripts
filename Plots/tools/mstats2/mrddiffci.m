function [rddiff,cilohi,p] = mrddiffci(r13,r23,r12,n,alpha,method)
% function [rddiff,cilohi,p] = mrddiffci(r13,r23,r12,n,alpha)
% 
...
% method    determines the method to compute the p-value
%           1: according to Steiger (1980), Psychol Bulletin, as described in Diehl & Arbinger
%           2: according to Bortz
%           in some tests, both methods yielded highly similar results; however, method 1 can generate errors because
%           W can turn negative
% 
% OUTPUTS
% rdiff     abs(r13-r23)
...
% 
% EXAMPLE
% [rddiff,cilohi,p] = mrddiffci(-0.336,-0.126,0.413,342,0.05,1)

% FORMULAS TAKEN FROM: 
% Diehl and Arbinger, Einf?hrung in die Inferenzstatistik (2nd edition), pp. 382
...
% 
% HISTORY
% 2014, Jan     two methods for computing p-value, taken from two different books with slightly different outcomes
%               implemented because method 1 s-t yields an error message although code is correct (verified vs. book)
% 
...
cilohi    = [cilo,cihi];
%% compute p-value according to desired method
if method==1        % Diehl and Arbinger
  
  % verified with an example from the book: [rddiff,cilohi,p] = rddiffci(-0.336,-0.126,0.413,342,0.05), Tw=3.776
  rddiff   = r13-r23;
