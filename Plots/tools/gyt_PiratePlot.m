function handles = gyt_PiratePlot(X,Y,width,smoothingFactor,cutEdges,pcolor,spread,limits,scatterargs)
...
for iP = 1:numel(X)
    ...
    handles(end-1) = plot(xplot+[-xx(Ind),xx(Ind)],[nanmean(yplot),nanmean(yplot)],'color',darkcolor,'linewidth',2.5);
...
%         if Condition ==1
%         handles(end) = scatter(xplot+yy,yplot,2,'markerfacecolor',ccolor,'markeredgecolor','k');%k
        handles(end) = scatter(xplot+yy,yplot,2,'markerfacecolor',ccolor,'markeredgecolor','k', 'LineWidth',8);%k
%         end
