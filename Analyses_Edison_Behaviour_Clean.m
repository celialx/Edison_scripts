...
Computer = 'Home';
% Paths
if strcmp(Computer, 'Eole')
    SavePath = 'C:\Users\Célia\Desktop\WagnerEdison project\Analyses_Results\Edison_Tables\';
    DataPath = 'C:\Users\Célia\Desktop\WagnerEdison project\NRT_07_07_2020_Scripts\Results\';
    ...
    DataPath = 'C:\Users\OUDIETTE.Delphine\Desktop\WagnerEdison project\NRT_07_07_2020_Scripts\Results\';
    EEGPath = SavePath;
    SubInfoPath = 'C:\Users\OUDIETTE.Delphine\Desktop\WagnerEdison project\Analyses_Results';
...
for i_sub=1:length(AllSubjects)
    ...
    if strcmp(sujet, '57LL') % Pb script, did not record last trial of practice
    ...
    if strcmp(sujet, '47OK') % Nothing in mat, but data in post
        ...
        Edison_Behaviour(i_sub).LastBlock_Pre = nan;
    else
        ...
        Edison_Behaviour(i_sub).LastBlock_Pre = mean(Pre_T.Time(Pre_T.Block==2)); %last block of pre   
    end
    ...
    if Edison_Subject.InsightPre(i_sub) ==1
        ...
        %         AhaMoment = findchangepts(Edison_Behaviour(i_sub).TradeOff, 'Statistic', 'mean');
        %         Edison_Behaviour(i_sub).AhaMoment = AhaMoment;
        ...
        Edison_Behaviour(i_sub).RTNonSol_beforeAha = NaN;
        ...
        if strcmp(sujet, '25DS') % Pre solver, stopped after pre
            ...
            Edison_Behaviour(i_sub).FirstBlock_Post = nan;
            Edison_Behaviour(i_sub).RTNonSol_beforeAha = NaN;
            ...
            Edison_Behaviour(i_sub).FirstBlock_Post = mean(Post_T.Time(Post_T.Block==1)); %first block post
            Edison_Behaviour(i_sub).RTNonSol_beforeAha = NaN;
    ...
    elseif strcmp(sujet,'14CD') % Script stopped after insight
        ...
        Edison_Behaviour(i_sub).TradeOff_fromAha = Edison_Behaviour(i_sub).Correct_fromAha/Edison_Behaviour(i_sub).RT_fromAha;
        Edison_Behaviour(i_sub).TradeOff_beforeAha = Edison_Behaviour(i_sub).Correct_beforeAha/Edison_Behaviour(i_sub).RT_beforeAha;
        Edison_Behaviour(i_sub).RTNonSol_beforeAha = mean(cell2mat(Post_T.Time(1:94)));
        
        for t = 1:length(Post_T.Correct)
            Edison_Behaviour(i_sub).TradeOff(t) = cell2mat(Post_T.Correct(t))*100/cell2mat(Post_T.Time(t));
        end
        Edison_Behaviour(i_sub).FirstBlock_Post = mean(cell2mat(Post_T.Time(Post_T.Block==1)));
        ...
        Edison_Behaviour(i_sub).RTNonSol_beforeAha = mean(Post_T.Time(1:94));
        ...
        Edison_Behaviour(i_sub).FirstBlock_Post = mean(Post_T.Time(Post_T.Block==1));
...
save(strcat(SavePath,'All_Data_Behaviour'), 'Data');
