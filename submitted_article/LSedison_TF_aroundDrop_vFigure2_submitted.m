...
for nF=1:length(files)
    ...
    if ~isempty(find(T.Ball(:,1)))
        ...
        all_TF_drops=cat(1,all_TF_drops,mean(temp,1));
        ...
        if ~isempty(drops)
            ...
            %             end
            cfg=[];
            cfg.trialfun             = 'LSedison_trialfun_drops_perm';
            cfg.dataset             = [Folder_Name filesep  File_Name];
            cfg.trialdef.prestim    = 2*60;
            cfg.trialdef.poststim   = .5*60;
            cfg.demean              = 'yes';
            ...
            all_TF_drops_perm=cat(1,all_TF_drops_perm,(nanmean(temp_perm,1)));
...
[pV_slope(3)]=signrank(coeff_polyfit{1}((~isnan(SumDrops)),1),coeff_polyfit{2}((~isnan(SumDrops)),1));
