...
%             FOOOF2_Pow_allE(nc,nCh,:) = nan;;
    ...
    all_Pow = [all_Pow ; [nF mean(Pow_all(nc,freqs>2 & freqs<4)) mean(Pow_all(nc,freqs>4 & freqs<8)) mean(Pow_all(nc,freqs>8 & freqs<11)) mean(Pow_all(nc,freqs>1 & freqs<40)) nan(1,2) length(AASM_Score2) sum(AASM_Score2==0) sum(AASM_Score2==1) sum(AASM_Score2==2) ...
        mean((Pow_perStage(nc,1,freqs>2 & freqs<4))) mean((Pow_perStage(nc,1,freqs>4 & freqs<8))) mean((Pow_perStage(nc,1,freqs>8 & freqs<11))) mean((Pow_perStage(nc,1,freqs>1 & freqs<40))) nan(1,2)...
        mean((Pow_perStage(nc,2,freqs>2 & freqs<4))) mean((Pow_perStage(nc,2,freqs>4 & freqs<8))) mean((Pow_perStage(nc,2,freqs>8 & freqs<11))) mean((Pow_perStage(nc,2,freqs>1 & freqs<40))) nan(1,2)...
        mean((Pow_perStage(nc,3,freqs>2 & freqs<4))) mean((Pow_perStage(nc,3,freqs>4 & freqs<8))) mean((Pow_perStage(nc,3,freqs>8 & freqs<11))) mean((Pow_perStage(nc,3,freqs>1 & freqs<40))) nan(1,2)...
        mean((Pow_perStage(nc,4,freqs>2 & freqs<4))) mean((Pow_perStage(nc,4,freqs>4 & freqs<8))) mean((Pow_perStage(nc,4,freqs>8 & freqs<11))) mean((Pow_perStage(nc,4,freqs>1 & freqs<40))) nan(1,2) ]];
...
for nS=1:length(all_Subds)
    ...
    data_clean.PowDelta(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(1,1) & freqs2<=Freqs(1,2)),3));
    data_clean.PowTheta(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(2,1) & freqs2<=Freqs(2,2)),3));
    data_clean.PowAlpha(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(3,1) & freqs2<=Freqs(3,2)),3));
    
    data_clean.PowDelta_dev(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(1,1) & freqs2<=Freqs(1,2)),3));
    data_clean.PowTheta_dev(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(2,1) & freqs2<=Freqs(2,2)),3));
    data_clean.PowAlpha_dev(this_line)=squeeze(mean(Pow_allE(nS,3,freqs2>=Freqs(3,1) & freqs2<=Freqs(3,2)),3));
    ...
    data_clean.PowSlope(this_line)=nan; %all_Pow(nS,7);
    data_clean.PowBG(this_line)=nan; %all_Pow(nS,6);
    
    data_clean.AlphaPeakFreq(this_line)=nan; %all_AlphaPeaks(nS,5);
    data_clean.AlphaPeakAmp(this_line)=nan; %all_AlphaPeaks(nS,6);
...
if redo
    ...
    [realpos_quad realneg_quad]=get_cluster_permutation_aov(sqrt(zscore(temp_power).^2),[All_Conds2(~isnan(All_Conds)) All_Insight(~isnan(All_Conds))],...
        0.05,0.1,1000,freqs2);
    save('result_clusterperm_onfreq_ANOVA_noFOOOFpower','realpos_lin','realneg_lin','realpos_quad','realneg_quad')
else
    load('result_clusterperm_onfreq_ANOVA_noFOOOFpower')
end
%%
    temp_power=squeeze(Pow_allE((~isnan(All_Conds)),3,:));
    myfreqs=freqs2;
