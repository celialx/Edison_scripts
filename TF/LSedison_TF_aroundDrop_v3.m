...
COI=3;
figure; set(gcf,'Position',[57         338        1104         349]);
for nF=1:size(Freqs,1)
    subplot(1,3,nF);
    FOI=Freqs(nF,:);
    temp_toplot=squeeze(median(all_TF_drops(:,COI,freqs>=FOI(1) & freqs<=FOI(2),:),3));
...
xlim([.5 3.8])
set(gca,'XTick',1:3,'XTickLabel',TitleFreqs(1:3))
