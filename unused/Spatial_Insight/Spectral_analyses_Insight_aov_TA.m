...
%%
Freqs=[1 3.6; 7.8 10.6; 16.4 18.2];
...
Data.PowTheta=nan(size(Data,1),1);
...
for nCond=1:2
    for nCond2=1:2
        if nCond2==1
        temp_toplot=squeeze(temp_power(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}) & All_Memory(~isnan(All_Conds))<median(All_Memory(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}))),:));
        else
        temp_toplot=squeeze(temp_power(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}) & All_Memory(~isnan(All_Conds))>median(All_Memory(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}))),:));
        end
        if nCond2==1
...
for nCond=1:2
    for nCond2=1:2
        if nCond2==1
            temp_toplot=temp_power_zscore(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}) & All_Memory(~isnan(All_Conds))<median(All_Memory(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}))),:);
        else
            temp_toplot=temp_power_zscore(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}) & All_Memory(~isnan(All_Conds))>median(All_Memory(ismember(All_Conds(~isnan(All_Conds)),myConds{nCond}))),:);
        end
        if nCond2==1
...
for nFreq=1
    ...
    xlim([.5 5.5])
    set(gca,'XTick',[2 4],'XTickLabel',{'<median','>median'})
...
%%
anovan(nanzscore(Data.PowTheta),[Data.SleepNap Data.Corrprenotpost],'varnames',{'Sleep','Insight'},'model','linear');
anovan(nanzscore(Data.PowTheta).^2,[Data.SleepNap Data.Corrprenotpost],'varnames',{'Sleep','Insight'},'model','linear');
